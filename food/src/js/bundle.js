...
function forms() {
	...
	function showThanksModal(message) {
		...
		(0,_modal__WEBPACK_IMPORTED_MODULE_0__.openModal)();    // Метод отвечает за открытие модальных окон
...
function timer() {
	...
	const deadline = '2023-11-15T15:18:00';
	// deadline.setDate(deadline.getDate() + 3);        // Можно добавить 3 дня обратного отсчёта
	...
	function getTimeRemaining(endtime) {
		...
		if (t < 0) {
			...
			hours = Math.floor((t / (1000 * 60 * 60) % 24));           // Получаем остаток часов
	...
	function getZero(num) {                 // Функция для подставления нуля перед цифрой
		if (num >= 0 && num < 10) {
			return `0${num}`;
		} else {
			return num;
	...
	function setClock(selector, endtime) {
		...
		updateClock();                                          // Вызываем чтобы не было задержки на 1 секунду
